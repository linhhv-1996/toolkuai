...
<div class="w-full flex-1 flex-shrink-0">
        ...
        {:then posts}
            {#each posts as post}
                <a href="{langPath}/blog/{post.slug}" class="py-4 px-0 block group hover:bg-gray-50 transition-colors">
                    <div class="blog-content text-[15px] font-semibold text-slate-900 leading-snug mb-1.5 group-hover:text-[#10b981] transition-colors">
                        {post.title}
                    </div>
                    <div class="blog-content text-[13px] text-slate-600 leading-normal line-clamp-2 mb-2">
                        {post.description}
                    </div>
                    <div class="blog-content text-[11px] font-bold text-slate-500 uppercase tracking-tight">
    ...
    <a href="{langPath}/blog" class="text-[12px] font-bold text-gray-400 hover:text-[#10b981] block mt-4 transition-colors uppercase tracking-widest">
        {t.viewAll}
    </a>
</div>
