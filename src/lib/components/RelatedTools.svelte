    ...
    <div class="divide-y divide-gray-100 border-t border-b border-gray-100 bg-white">
        {#each relatedTools as tool}
            ...
            <a href="{langPath}/{metadata.slug}" class="py-4 px-0 block group hover:bg-gray-50 transition-colors">
                <div class="text-sm font-semibold text-slate-900 leading-snug mb-1.5 group-hover:text-[#10b981] transition-colors flex items-center gap-2">
                    <span>{metadata.title}</span>
                </div>
                <div class="text-[13px] text-slate-600 leading-normal line-clamp-2">
