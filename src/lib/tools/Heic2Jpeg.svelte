...
<script lang="ts">
    ...
    const overallProgress = $derived(
        Math.round(heicQueue.reduce((sum, item) => sum + (item.progress || 0), 0) / heicQueue.length)
    );
...
<div class="w-full">
    <div class="tool-box relative bg-white rounded-sm border border-gray-200 p-6 space-y-6" 
         style="height: 430px; overflow: hidden; box-sizing: border-box; width: 100%;">
        {#if status === "idle"}
            <div class="space-y-6">
                <Dropzone onFilesSelected={handleFiles} accept=".heic,.HEIC" {t} />
